# Panduan Lengkap PBAK One

Ikut langkah mengikut turutan. Jangan lompat langkah.

## 1. Ekstrak fail ZIP

1. Ekstrak fail `pbak-one.zip`.
...
2. Klik **New project**.
3. Pilih organisasi.
4. Masukkan nama projek: `pbak-one`.
...
12. Masukkan nama: `PBAK One Web`.
13. Dalam **Authorized JavaScript origins**, tambah:
    - `http://localhost:5173`
    - Domain Vercel anda selepas deployment, contoh `https://pbak-one.vercel.app`
...
8. Buka folder `pbak-one` yang telah diekstrak.
...
2. Buka Google OAuth client `PBAK One Web`.
